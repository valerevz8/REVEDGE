...
function buildHalverInput(event: any) {
  ...
  const macroPressure = keywordScore(text,
    ["hawkish", "rate hike", "hike", "higher rates", "tightening", "hot inflation", "strong jobs", "yield up", "dollar strength"],
    ["dovish", "rate cut", "cut", "lower rates", "easing", "cool inflation", "weak jobs", "yield down", "dollar weakness"],
  );
  const surprise = keywordScore(text,
    ["beat", "beats", "above", "higher than expected", "stronger than expected", "accelerates", "surge", "inflow", "approval"],
    ["miss", "misses", "below", "lower than expected", "weaker than expected", "slows", "drop", "outflow", "rejection"],
  );
  const expectation = keywordScore(event.bias ?? text,
    ["bullish", "risk-on", "upside", "positive", "supportive", "breakout", "accumulation"],
    ["bearish", "risk-off", "downside", "negative", "headwind", "breakdown", "distribution"],
  );
  ...
  return {
    market: {
      regime: clamp(direction * 0.65),
      expectations: clamp(expectation || direction * 0.45),
      positioning,
      policyPressure: macroPressure,
      surprise: surprise || direction * 0.25,
      transmission,
      btcStructure: structure,
    },
    dataQuality: clamp(0.55 + corroboration * 0.25 + Math.min(0.2, Number(event.confirmation ?? 0) / 500), 0, 1),
  };
}
...
function halverPredictive(event: any) {
  ...
  const invalidation = [
    result.invalidation,
    event.invalidation,
    ...(event.triggerRows ?? []).map((row: any) => row.invalidation),
  ].filter(Boolean).slice(0, 3);

  return {
    directionScore,
    executionScore: result.tradeability ?? Math.max(35, Math.round(Math.abs(result.biasScore) * 0.7)),
    riskFlag: risk.riskFlag,
    riskLevel: risk.riskLevel,
    riskLabel: result.thesisStatus === "PENDING" ? `${result.edgeQuality.toLowerCase()} pre-event edge` : `${result.thesisStatus.toLowerCase()} thesis`,
    ...
    reactionRead: result.reactionRead,
    transmissionRead: result.transmissionRead,

  ...
  return (
    <section className="shell section" id="impact">
      <div className="sectiontitle">
        <div><div className="label">Decision intelligence · Priority feed</div><h2>High Impact</h2></div>
      ...
      <div className="grid">
        <article className="card impact">
          ...
          <h3 className="impactheadline">{headline}</h3>
          ...
          <div className="decisionstrip"><span><b>Priority</b> 🔥 {story.impact.toFixed(0)}/10</span><span><b>Urgency</b> {story.urgency === "NOW" ? "WATCH CLOSELY" : "MONITOR"}</span><span><b>Regime</b> {story.regime}</span><span><b>Bias</b> {story.bias}</span></div>
        ...
        <div className="card">
          ...
          <div className="decisioncall"><span className="label">Immediate read</span><b>{story.finalAction}</b></div>
        </div>
      </div>

      <div className="card timingcard">
        <div className="label">Event clock</div>
        <div className="timinggrid">
          <div><span>Event / alert time</span><b>{zoneTime(story.publishedAt, "Asia/Jakarta", "WIB")}</b></div>
          <div><span>U.S. time</span><b>{zoneTime(story.publishedAt, "America/New_York", "ET")}</b></div>
          <div><span>UK time</span><b>{zoneTime(story.publishedAt, "Europe/London", "UK")}</b></div>
          <div><span>Market session</span><b>{sessionFor(story.publishedAt)}</b></div>
          <div><span>Alert age</span><b>{ageLabel(story.publishedAt)}</b></div>
          <div><span>Expected relevance</span><b>{story.window}</b></div>
        </div>
        <p className="microcopy">For scheduled events, REVEDGE should eventually show the <b>future event time + countdown</b> here; this live feed currently anchors the clock to the verified alert timestamp.</p>
      </div>
      ...
      <div className="highdetail">
        ...
        <article className="card actionbox">
          ...
          <div className="actionlist">
            ...
            <div className="actionrow"><strong>02</strong><span><b>Trigger:</b> {story.triggerRows[0]?.trigger ?? "Wait for price confirmation."}</span></div>
            <div className="actionrow"><strong>03</strong><span><b>Confirmation:</b> {story.triggerRows.slice(1, 3).map((r) => r.watch + " → " + r.trigger).join(" · ")}</span></div>
            <div className="actionrow"><strong>04</strong><span><b>Invalidation:</b> {story.triggerRows[0]?.invalidation ?? "BTC fails the move and breadth contracts."}</span></div>
          </div>
      ...
      <div className="card watchcard">
        ...
        <div className="watchtable">
          <div className="watchhead"><span>WATCH</span><span>TRIGGER / CONFIRMATION</span><span>INVALIDATION</span></div>
          {story.triggerRows.map((row, i) => <div className="watchrow" key={`${row.watch}-${i}`}><b>{row.watch}</b><span>{row.trigger}</span><span>{row.invalidation}</span></div>)}
        </div>
      </div>

      <div className="card positioncard">
        <div className="label">Position-aware decision</div>
        <div className="positiongrid">
          <div><span>NO POSITION</span><b>{actions.flat}</b></div>
          <div><span>OPEN LONG</span><b>{actions.long}</b></div>
          <div><span>OPEN SHORT</span><b>{actions.short}</b></div>
        </div>
      </div>

      <div className="finalaction">
        <div className="label">Final action</div>
        <h3>{story.finalAction}</h3>
        <p>{story.tradableSetup}</p>
        <div className="scenario"><span className="scenario-bull">🟢 Bull recovery</span><b>{story.bullCase}</b></div>
        <div className="scenario"><span className="scenario-bear">🔴 Rejection</span><b>{story.bearCase}</b></div>
      </div>
    </section>
  );
}

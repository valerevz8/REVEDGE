...
type TriggerRow = { watch: string; trigger: string; invalidation: string };
type Event = {
  ...
  tradableSetup: string; triggerRows: TriggerRow[]; related: Related[];
};
...
const ID: Record<string, string> = {
  ...
  "High Impact": "Dampak Tinggi", "Lifecycle": "Siklus event", "Corroboration": "Konfirmasi sumber",
  "independent sources": "sumber independen", "Impact window": "Jendela dampak", "State change": "Perubahan status",
  "Related coverage": "Liputan terkait", "Why it matters": "Kenapa ini penting", "What to watch": "Yang perlu dipantau",
  "Impact intelligence": "Inteligensi dampak", "Trader guidance": "Panduan trader", "What to do": "Apa yang dilakukan",
  "Event age": "Usia event", "Confidence": "Keyakinan", "Market impact": "Dampak market", "Direction": "Arah",
  "Market condition": "Kondisi market", "Main driver": "Penggerak utama", "Priority": "Prioritas", "Sources": "Sumber",
  "Confirmation": "Konfirmasi", "NEW": "BARU", "ACTIVE": "AKTIF", "ESCALATED": "NAIK",
  "CONFIRMED": "TERKONFIRMASI", "UNCHANGED": "TETAP", "FADING": "MEREDA", "DOWNGRADED": "TURUN",
  "NOW": "SEKARANG", "WATCH": "PANTAU", "old": "lalu", "No high-impact event is available right now.": "Belum ada event berdampak tinggi yang cukup kuat saat ini.",
  "Trade the confirmation, not the headline.": "Trade setelah konfirmasi, bukan karena headline-nya.",
  "Risk-off": "Risk-off", "Risk-on": "Risk-on", "Neutral": "Netral", "Cautious": "Waspada",
  "Mixed": "Campuran", "Bullish": "Bullish", "Bearish": "Bearish", "Reclaim key BTC level + hold": "Rebut kembali level kunci BTC lalu bertahan",
  "Lose key BTC support": "Kehilangan support kunci BTC", "Reclaim key ETH level + outperform BTC": "Rebut kembali level kunci ETH dan unggul dari BTC",
  "Continued ETH underperformance": "ETH terus tertinggal", "Reclaim key SOL level": "Rebut kembali level kunci SOL",
  "Sustained SOL weakness": "SOL tetap lemah", "BTC": "BTC", "ETH": "ETH", "SOL": "SOL",
  "USD / Yields": "USD / Yield", "TOTAL2 / TOTAL3": "TOTAL2 / TOTAL3",
};
...
function valueOrDash(value: unknown) { return value === undefined || value === null || value === "" ? "—" : String(value); }
    ...
    {!event ? <div className="card newsempty">{tr("No high-impact event is available right now.", id)}</div> : <>
      <div className="grid">
        <article className="card impact" style={{ position: "relative", overflow: "hidden" }}>
          ...
          <div className="impactscoreline"><strong>{event.impact.toFixed(1)} / 10</strong><span>{id ? tr(event.direction, true) : event.direction}</span></div>
        ...
        <aside className="card"><div className="label">{tr("Corroboration", id)}</div><h3>{event.corroboration} {tr("independent sources", id)}</h3><div className="bar" style={{ marginTop: 14 }}><div className="fill" style={{ width: `${event.confirmation}%` }} /></div><div className="windowline"><span>{event.confirmation}% {tr("Confirmation", id)}</span><span>{event.sourceCount} {tr("Sources", id)}</span></div><div className="decisioncall" style={{ marginTop: 18 }}><span className="label">{tr("Direction", id)}</span><b>{id ? tr(event.direction, true) : event.direction}</b></div><div className="decisioncall"><span className="label">{tr("Market condition", id)}</span><b>{id ? tr(event.regime, true) : event.regime}</b></div><div className="decisioncall"><span className="label">Bias</span><b>{id ? tr(event.bias, true) : event.bias}</b></div></aside>
      </div>
      ...
      <div className="card" style={{ marginTop: 14 }}>
        ...
        <div className="decisionstrip" style={{ marginTop: 12 }}>
          <span><b>{tr("Event age", id)}</b>{ageLabel(event.ageHours, id)}</span>
          <span><b>{tr("Confidence", id)}</b>{event.confirmation}%</span>
          <span><b>{tr("Market impact", id)}</b>{event.impact.toFixed(1)} / 10</span>
          <span><b>{tr("Direction", id)}</b>{id ? tr(event.direction, true) : event.direction}</span>
        </div>
        <div className="decisionstrip" style={{ marginTop: 10 }}>
          <span><b>{tr("Market condition", id)}</b>{id ? tr(event.regime, true) : event.regime}</span>
          <span><b>Bias</b>{id ? tr(event.bias, true) : event.bias}</span>
          <span><b>{tr("Priority", id)}</b>{Math.round(event.priority)}/10</span>
      ...
      <div className="card" style={{ marginTop: 14 }}>
        ...
        <h3 style={{ marginBottom: 12 }}>{tr("What to do", id)}</h3>
        <div className="actionlist">
          <div className="actionrow"><strong>01</strong><span>{id ? "Tanpa posisi — tunggu harga mengonfirmasi arah. Headline saja belum cukup jadi setup." : "No position — wait for price to confirm direction. The headline alone is not a setup."}</span></div>
          <div className="actionrow"><strong>02</strong><span>{id ? `Long terbuka — jaga risiko tetap terkontrol dan naikkan stop hanya setelah harga benar-benar melanjutkan arah.` : "Open long — keep risk controlled and move the stop only after price confirms continuation."}</span></div>
          <div className="actionrow"><strong>03</strong><span>{id ? `Short terbuka — jangan tambah ukuran posisi sebelum tekanan turun dikonfirmasi BTC dan breadth.` : "Open short — do not add size until downside pressure is confirmed by BTC and breadth."}</span></div>
          <div className="actionrow"><strong>04</strong><span>{id ? `Validasi: ${valueOrDash(event.tradableSetup)}` : `Validation: ${valueOrDash(event.tradableSetup)}`}</span></div>
        </div>
      </div>

      <div className="card" style={{ marginTop: 14 }}><div className="label">{tr("What to watch", id)}</div><div className="actionlist">{event.triggerRows.slice(0, 4).map((row, i) => <div className="actionrow" key={`${row.watch}-${i}`}><strong>{String(i + 1).padStart(2, "0")}</strong><span><b>{id ? tr(row.watch, true) : row.watch}</b><br />{id ? tr(row.trigger, true) : row.trigger}<br /><span className="muted">{id ? tr(row.invalidation, true) : row.invalidation}</span></span></div>)}</div></div>
      ...
      <div className="card actionbox" style={{ marginTop: 14 }}><div className="label">REVEDGE V1</div><h3>{id ? (ID[event.finalAction] ?? event.finalAction) : (event.finalAction || tr("Trade the confirmation, not the headline.", id))}</h3><p className="muted">{event.tradableSetup}</p></div>
    </>}
  </section>;
}
